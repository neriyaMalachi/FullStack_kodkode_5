...
בפרק הבא נתחיל יחידה חדשה — **Docker**. עד עכשיו הרצנו את מסד הנתונים, את השרת, ואת כל התלויות ישירות על המחשב שלנו — וזה עובד, אבל "אצלי זה עבד" הופך לבעיה אמיתית כשמישהו אחר מנסה להריץ את אותו פרויקט. Docker פותר את זה: אורז את כל הסביבה (קוד + תלויות + הגדרות) לכדי חבילה אחת שרצה זהה בכל מקום.

...
בפרק הבא נתחיל יחידה חדשה — **Docker**. עד עכשיו הרצנו את השרת ישירות על המחשב שלנו — וזה עובד, אבל "אצלי זה עבד" הופך לבעיה אמיתית כשמישהו אחר מנסה להריץ את אותו פרויקט. Docker פותר את זה: אורז את כל הסביבה (קוד + תלויות + הגדרות) לכדי חבילה אחת שרצה זהה בכל מקום — ומכין אתכם להריץ גם את מסד הנתונים עצמו כקונטיינר ביחידה שאחריה.

...
בפרק הבא נלמד על **Logging** — לאורך כל היחידה השתמשנו ב-`console.log` לדיבוג — וזה בסדר גמור בפיתוח מקומי. אבל בשרת חי (production), `console.log` לא מספיק: הוא לא נשמר לאורך זמן, לכל השורות יש אותה "חשיבות" (אין הבדל בין מידע שגר
